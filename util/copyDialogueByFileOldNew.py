...
from service.dialogue_service import getDialogueIdByContextOld
...
# Excel导入流程
# 京东-新平台创建对话流程
def createSlotsNew(workspace_id):
    readBook = xlrd.open_workbook(r'../excel/兴安贷初版话术流程.xlsx')
    # sheetFaq = readBook.sheet_by_index(0)
    sheetFaq = readBook.sheet_by_name("XMGJ兴安贷A")
    ...
    for i in range(INDEX_START, INDEX_END):
        ...
        if number != '' and constants.ACTION_END not in action:
            ...
            # 结果节点（跳转）
            node_id = long(getDialogueIdByContextOld(workspace_id, content, cookie))
            ...
            nodeDataArray.append(nodeDataEnd)
            nextLink = addLinkData(cur_node_key+1, cur_node_key, "", "", 0)
            linkDataArray.append(nextLink)
    flow = {
...
def addLinkData(from_key=None, to_key=None, desc="为空", title="", number=0, dialogue_round=0):
    ...
    if "首句" in title:
        ...
        compare_type = "EQUALS"
        value = "1"
    elif "不需要-使用其他平台" not in title and ("其他" in title or "无明确回应" in title or "" == title or '' == title or title is None):
        desc = "为空"
...
# 从文档中生成对话流程
def dealWorkspace(workspace_id, dialogue_id, cookie):
    flow = createSlotsNew(workspace_id)
    ...
    for index in range(1, len(nodeDataArray)):
        ...
        if str(data["text"]) == "结果":
            if str(data["type"]) == "RETURN_DIALOGUE":
                ...
                newFinalAct = slot_dao.setFinalActOld("BUSINESS_SYSTEM", reply, workspace_id, cookie)
            ...
            slot = slot_dao.createSlotOld(data, reply, workspace_id, dialogue_id, cookie)
...
if __name__ == '__main__':
    cookie = "JSESSIONID=node01ib6thvzl4xrc1cl4tenmyrihv19858346.node0"
    dealWorkspace(360, 2317, cookie)
    # createSlotsNew()
