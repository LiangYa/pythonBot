...
def createSlotsNew():
    ...
    for i in range(INDEX_START, INDEX_END):
        ...
        if number != '' and constants.ACTION_END not in action:
            ...
            linkDataArray.append(curLink)
            cur_node_key = cur_node_key - 1
            nodeDataEnd = addEndNode(cur_node_key, dialogue_branch, dialogue_round * 2 + 1, "JUMP_DIALOGUE")
            nodeDataArray.append(nodeDataEnd)
...
# 加结果节点
def addEndNode(k, dialogue_branch, dialogue_round, node_type="LEAF"):
    nodeData = {
        "text": "结果",
        "id": -2,
        "figure": "RoundedRectangle",
...
if __name__ == '__main__':
    cookie = "JSESSIONID=node01wob4dxhntvps83lnzvd771jt2224763.node0"
    dealWorkspace(350, 74691, 2242, cookie)
    # createSlotsNew()
